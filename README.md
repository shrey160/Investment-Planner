...
An interactive compound interest calculator that lets you model, compare, and stress-test multiple investment portfolios side by side — with inflation-adjusted deposits, major spend events, and multi-currency display.

---

## Getting started

The tool opens with two portfolios pre-loaded. Everything is live — any change you make to a slider or toggle updates the chart and summary cards instantly.
...
The event appears as a card showing which portfolios are affected and by how much. Click **×** on any event card to remove it.
...
When you have more than one portfolio, toggle **Show total** (above the chart on the right) to display a dashed grey line representing the combined balance of all portfolios. A **Total** card also appears in the summary row at the top.
...
1. Click **Custom currency** in the top-right of the tool.
...
All values across the chart, summary cards, portfolio stats, and spend events update immediately. The underlying data never changes — conversion is display only.
